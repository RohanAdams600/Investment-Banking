...
The name and tagline are settled (**Ashlar** / "The marketplace for buying and selling businesses"). `BRAND_SUPPORT_EMAIL` and
...
| ------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CSP_ENFORCE` | No | **Enforced by default.** Set it to `false` only for a rollout window in which you are reading violation reports, then remove it — `preflight --strict` fails while it is off. Note the constraint this places on rendering: the policy is nonce-based, the nonce is minted per request in middleware, and a statically prerendered page's HTML predates it — so any route that must work under enforcement has to render dynamically. `/`, `/sell`, `/pricing`, `/sign-in` and `/sign-up` carry `export const dynamic = 'force-dynamic'` for exactly this reason. A new statically prerendered page with client JavaScript will have every script refused. |

| `CRON_SECRET` | **Yes** | Bearer token for all three scheduled routes: `/api/cron/due-tasks`, `/api/cron/saved-search-alerts` and `/api/cron/purge`. Unset means every one of them refuses every caller — an unset secret is a misconfiguration, not permission — so reminders stop, saved-search alerts stop, and expired confidential data is never deleted. Set it to a long random string and give the same value to whatever calls the routes. All three are declared in `vercel.json`; Vercel Cron issues a `GET` and sends this as `Authorization: Bearer`. |
